...
int main() {
    ...
    while ( true ) {
        ...
        bool command = false;
        for (auto oper : COMMANDS)

        {
            bool check = false;
            if (command_to_be_executed.length() == oper.str.length())
            {
                check = true;
                for (unsigned int i =0; i < command_to_be_executed.length(); i++ )
                {
                    if (command_to_be_executed[i]-32 != oper.str[i] && command_to_be_executed[i] != oper.str[i] )
                    {
                        check = false;
                        break;
                    }
            ...
            {
                    command = true;
                    if (pieces.size() -1 != oper.parameter_number)
                    {
                    ...
                    //std::cout<<i<<"\n";
                    string_to_double(pieces.at(2),j);
                    //std::cout<<i<<"\n";
                    if (!(i&&j))
                    {
                    ...
                    {
                        std::cout<<oper.action(std::stod(pieces.at(1)),std::stod(pieces.at(2)))<<"\n";
                        break;
                    }
            }
            {
